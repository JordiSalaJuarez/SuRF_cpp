...
enum Type { Fragmented, NotFragmented };

std::vector<std::string> get_input_data(size_t n_keys, Type kind){
    std::ifstream file;
    if (kind == Type::Fragmented){
...
static void BM_ReadVector(benchmark::State& state){
    std::ifstream file;
    std::vector<std::string> keys = get_input_data(state.range(1), Type(state.range(0)));
    for (auto _ : state){
        state.PauseTiming();
...
static void BM_ReadFile(benchmark::State& state){
    ...
    for (auto _ : state){
        state.PauseTiming();
        std::vector<std::string> keys;
        if (state.range(0) == Type::Fragmented){
            file = std::ifstream("data/keys_100M");
        }else{
            file = std::ifstream("data/keys_100M");
        }
